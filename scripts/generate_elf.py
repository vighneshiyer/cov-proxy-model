...
def generate_elf(riscv_dv_bin, config, out_dir, out_prefix):
    riscv_dv_path = (Path.cwd() / "riscv-dv").resolve()
    riscv_dv_bin = Path(riscv_dv_bin).resolve()
    asm_file = Path(out_dir) / f"{out_prefix}"
    asm_file_out = Path(out_dir) / f"{out_prefix}_0.S"
    gen_log_file = Path(out_dir) / f"{out_prefix}.gen.log"
    spike_log_file = Path(out_dir) / f"{out_prefix}.spike.log"

    # Generate asm file
    (cmd, envvars) = riscv_dv_cmd(generator_bin=riscv_dv_bin,
                                  config=RiscvDvConfig.parse_raw(Path(config).read_text()),
    ...
    # Generate elf
    (cmd, envvars) = gcc_cmd(asm_file_out, riscv_dv_path, f"{out_prefix}.elf")
    ...
    (cmd, envvars) = spike_cmd(Path(out_dir) / f"{out_prefix}.elf")
    ...
    spike_status_file = (Path(out_dir) / f"{out_prefix}.spike.status")
    if timeout:
...
def main():
    ...
    generate_elf(args.riscv_dv_bin, args.config, args.out_dir, args.out_prefix)
